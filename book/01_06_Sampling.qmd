...
Also note that both resampling methods provide imperfect estimates, and can give different numbers. Percentiles of jackknife resamples are systematically less variable than they should be. Until you know more, a conservative approach is to take the larger estimate.
...
A famous theoretical result in statistics is that with independent and identical data, i.e. that each $X_{i}$ has the same mean $\mu$ and variance $\sigma^2$, 
\begin{eqnarray}
...
Notice that each additional data point you have provides more information, which ultimately decreases the standard error of your estimates. However, it does so at a decreasing rate (known in economics as diminishing returns).
