...
Scatterplots are used frequently to summarizes the *joint distribution* of multiple observations of $(\hat{X}_{i}, \hat{Y}_{i})$. They can be enhanced in several ways. As a default, use semi-transparent points so as not to hide any points (and perhaps see if your observations are concentrated anywhere). You can also add other features that help summarize the relationship, although I will defer this until later.
...
These describe the relationship between $\hat{Y}_{i}$ and $\hat{X}_{i}$. We show how distribution or density of one variable changes according to a second variable, often using data splits. E.g., 
