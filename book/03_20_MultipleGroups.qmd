...
Rather than relying on the $F_{G-1,\;n-G}$ distribution, we can bootstrap the $F$-statistic directly. The pairs bootstrap resamples entire observations (Murder, Region) with replacement, preserving whatever joint structure exists in the data.

```{r}
# Observed F-statistic
fit_aov   <- aov(Murder ~ Region, data = dat)
F_obs     <- summary(fit_aov)[[1]]['Region', 'F value']
...
F_boot <- replicate(B, {
    ...
    dat_boot <- dat[idx, ]
    summary(aov(Murder ~ Region, data = dat_boot))[[1]]['Region', 'F value']
})
...
p_param <- summary(fit_aov)[[1]]['Region', 'Pr(>F)']
cbind(F_obs, p_param, p_boot)
```

```{r}
hist(F_boot, breaks = 40,
    col = hcl.colors(1, alpha = .45), border = NA,
    freq = F, main = NA, xlab = 'Bootstrap F-statistic',
    xlim = c(0, max(F_obs * 1.2, quantile(F_boot, .99))))
title('Bootstrap F-test', font.main = 1)
abline(v = F_obs, col = rgb(1, 0, 0, .8), lwd = 2)
legend('topright', legend = 'Observed F', col = rgb(1, 0, 0, .8), lwd = 2)
```
:::
