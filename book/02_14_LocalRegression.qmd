...
A *regressogram* splits $X$ into exclusive intervals (bins) of half-width $h$ and reports the average of $\hat{Y}_{i}$ within each bin. It is the conditional-mean analog of a histogram.
:::
```{r}
# Data
plot(wage ~ school, data=Wages1, pch=16, col=grey(0, .1),
...
:::{.callout-important title="Definition" collapse="false"}
The *bandwidth* $h$ is the half-width of a local bin around the design point $x$: an observation is "local" to $x$ when $|\hat{X}_{i}-x|\leq h$. Larger $h$ pools more observations but smooths over real variation; smaller $h$ tracks the data closely but uses fewer points per estimate.
:::
...
Consider a point $x$ and model where values are constant around $x$;  $\hat{Y}_{i} = b_{0}(x, h) + e_{i}$.
Then notice a weighted OLS estimator with uniform kernel weights yields

:::{.callout-important title="Definition" collapse="false"}
A *locally constant (kernel) regression* fits a single constant at each design point $x$ using only observations within bandwidth $h$; equivalently, a weighted mean with kernel weights $k(\hat{X}_{i}, x, h)$. With a uniform kernel and exclusive bins it reduces to a regressogram.
:::
...
A smaller $\hat{MSE}$ on the data you fit is not automatically better.
A regressogram with one bin per observation fits every point exactly ($\hat{MSE}=0$), but it has just copied the data and would predict new observations poorly.
This is the *bias-variance tradeoff*, and the number of bins (equivalently, the half-width $h$) is the lever:

:::{.callout-important title="Definition" collapse="false"}
The *bias-variance tradeoff* describes how a model's expected error decomposes into two sources: *bias* (systematic miss from over-smoothing) and *variance* (sample-to-sample wobble from under-smoothing). The bandwidth $h$ governs the tradeoff: large $h$ trades bias up for variance down, small $h$ does the opposite.
:::
