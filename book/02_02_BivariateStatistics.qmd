...
Suppose $X$ and $Y$ are both *categorical* variables; the value of $X$ is one of $1...K$ categories and the value of $Y$ is one of $1...J$ categories. We organize such data as a contingency table with $K$ rows and $J$ columns and use *Cramer's V* to quantify the strength of association by adjusting a "chi-squared" statistic to provide a measure that ranges from $0$ to $1$; $0$ indicates no association while a value closer to $1$ signifies a strong association. 
...
n <- nrow(xy)
bootstrap_KT <- vector(length=9999)
for(b in seq(bootstrap_KT) ){
    xy_b <- xy[sample(n, replace=T),]
    KT_b <- cor(xy_b[, 1], xy_b[, 2], method="kendall")
    bootstrap_KT[b] <- KT_b
}
hist(bootstrap_KT, breaks=100,
    border=NA, font.main=1,
    main='')

## Test whether KT is statistically different from 0
boot_ci <- quantile(bootstrap_KT, probs=c(0.025, 0.975))
abline(v=boot_ci)
abline(v=0, col='red')
```
