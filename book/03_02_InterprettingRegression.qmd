...
The fourth plot further assesses outlier $X$ using *Cook's Distance*, which sums of all prediction changes when observation $i$ is removed and scales proportionally to the mean square error
\begin{eqnarray}
D_{i} 
...
1. For regressograms, you can approximate gradients with small finite differences. For some small $h_{p}$, we can manually compute
\begin{eqnarray}
\widehat{\beta_{p}}(\mathbf{x}) &=& \frac{ \widehat{Y}(x_{1},...,x_{p}+h_{p}...,x_{P})-\widehat{Y}(x_{1},...,x_{p}-h_{p}...,x_{P})}{2h_{p}},
\end{eqnarray}

2. When using split-sample regressions, you can get all estimated coefficients that provides gradient estimates in each direction.
\begin{eqnarray}
...
* Plot of gradients and CI's, \cite{Chaudhuri1999, HendersonEtAl2012}
