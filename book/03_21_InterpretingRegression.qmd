...
#### **Outliers**. {-}
The first diagnostic plot (residuals vs fitted) examines outliers in the outcome: the observed $\hat{Y}_i$ is far from its prediction $\hat{y}_i$. You may be interested in such outliers because they can (but do not have to) unduly influence your estimates.

The fourth diagnostic plot (residuals vs leverage) examines another type of outlier: the observed explanatory variable $\hat{X}_{i}$ is far from the others. A point has high *leverage* if the estimates change dramatically when you estimate the model without that data point.

:::{.callout-important title="Definition" collapse="false"}
The *leverage* $h_i$ of an observation is the diagonal element of the OLS projection matrix $\hat{P} = X(X'X)^{-1}X'$ at row $i$. High leverage means the point sits far from the others in $X$-space, so removing it can move the fitted line a lot.
:::
```{r}
N <- 40
...
:::{.callout-important title="Definition" collapse="false"}
The *standardized residual* $r_i$ is the residual divided by its estimated standard deviation (using a leave-one-out variance estimate). The scaling makes residuals comparable across observations with different leverages.
:::

The fourth plot further assesses outliers in the explanatory variables ($X$) using *Cook's Distance*, which sums all prediction changes when observation $i$ is removed and scales proportionally to the mean square error
\begin{eqnarray}
D_{i} 
...
\end{eqnarray}
:::{.callout-important title="Definition" collapse="false"}
*Cook's Distance* measures how much the fitted values change when observation $i$ is removed. Large values flag points that strongly influence the entire regression, not just their own fitted value.
:::
...
:::{.callout-important title="Definition" collapse="false"}
*Collinearity* is when one explanatory variable can be closely predicted by a linear combination of the others. The *Variance Inflation Factor* $\hat{VIF}_k = 1/(1-\hat{R}^2_k)$ quantifies how much it inflates the variance of $\hat{\beta}_k$ compared to the no-collinearity case.
:::
```{r}
...
*Heteroskedasticity* may also matter for variability estimates. The third diagnostic plot (scale-location) is one visual check; the *Breusch-Pagan test* below is a numerical alternative.

:::{.callout-important title="Definition" collapse="false"}
*Heteroskedasticity* is when the variance of the residuals depends on the regressors. It leaves the OLS coefficients unbiased but makes the conventional standard errors unreliable.
:::
```{r}
...
:::{.callout-important title="Definition" collapse="false"}
The *Box-Cox transformation* is a family of power transformations indexed by a parameter (here $\lambda$ for the outcome and $\rho$ for the regressors). It nests the linear ($\rho=\lambda=1$) and log ($\rho=\lambda=0$) specifications as special cases and is fit by minimizing prediction error on the original scale.
:::
\begin{eqnarray}
...
This chapter audited a fitted regression with four diagnostic plots (outliers, leverage, normality, scale), introduced the Variance Inflation Factor for collinearity, the Breusch-Pagan test for heteroskedasticity, and Box-Cox transformations as a way to rescue a misspecified linear model. In the next chapter we keep the multivariate setup but drop the linear assumption: local regressions and regressograms estimate the conditional mean without committing to a single line.
:::
