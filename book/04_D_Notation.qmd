...
| $\hat{\beta}_p(\mathbf{x})$ | Local marginal effect |
| $\mathbf{H}$ | Bandwidth matrix |

The letter $h$ is a bandwidth everywhere it appears: a histogram bin width, a kernel density bandwidth, and a local regression window are all the same idea, which is how much data around a point gets used.
