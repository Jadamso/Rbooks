...
```{r, echo=FALSE}
X0 <- sample(c(0,1), 400, prob=c(1/2, 1/2), replace=T)
...
Note that the [Discrete Uniform](https://en.wikipedia.org/wiki/Discrete_uniform_distribution) distribution generalizes to arbitrary intervals, such as $\{0.0, 0.25, 0.5, 0.75, 1.0\}$ instead of $\{1,2,3,4,5\}$. We will not exploit the generalization in this class.
