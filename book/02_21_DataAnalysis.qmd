...
for (i in seq_along(vars)) {
  ...
  stats_list[[i]] <- c(
    Variable = v,
    N       = sum(!is.na(x_hat)),
    Mean    = mean(x_hat, na.rm = TRUE),
    SD      = sd(x_hat, na.rm = TRUE),
    Min     = min(x_hat, na.rm = TRUE),
    Q1      = as.numeric(quantile(x_hat, 0.25, na.rm = TRUE)),
    Median  = median(x_hat, na.rm = TRUE),
    Q3      = as.numeric(quantile(x_hat, 0.75, na.rm = TRUE)),
    Max     = max(x_hat, na.rm = TRUE)
  )
}
...
```{r}
# Random Data
x_hat <- seq(1, 10, by=.0002)
e <- rnorm(length(x_hat), mean=0, sd=1)
y_hat <- .25*x_hat + e
...
x_std_hat <- scale(x_hat)
y_std_hat <- scale(y_hat)
plot(y_std_hat, x_std_hat,
    xlab='', ylab='',
...
The next chapter opens Part 3 with multiple linear regression, extending the single-predictor model from [Simple Regression](02_15_SimpleRegression.qmd) to many explanatory variables at once.
