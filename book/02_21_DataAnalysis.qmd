...
Color and size can carry two more variables, and the hover text can list every variable for the state under the cursor.
See also <https://plotly.com/r/bubble-charts/>.
```{r}
# Bubble chart: color and size both encode the murder rate
fig <- plot_ly(
    USArrests, x = ~ UrbanPop, y = ~ Assault,
    mode='markers',
    type='scatter',
...
fig <- add_histogram(fig, y1_hat, name='Low Pop. (< Mean)',
    histnorm = 'probability density',
    xbins = list(start=0, size=2))
fig <- add_histogram(fig, y2_hat, name='High Pop (>= Mean)',
    histnorm = 'probability density',
    xbins = list(start=0, size=2))
fig <- layout(fig,
    barmode='overlay',
...
for (i in seq_along(vars)) {
  ...
  stats_list[[i]] <- c(
    Variable = v,
    N       = sum(!is.na(x_v_hat)),
    Mean    = mean(x_v_hat, na.rm = TRUE),
    SD      = sd(x_v_hat, na.rm = TRUE),
    Min     = min(x_v_hat, na.rm = TRUE),
    Q1      = as.numeric(quantile(x_v_hat, 0.25, na.rm = TRUE)),
    Median  = median(x_v_hat, na.rm = TRUE),
    Q3      = as.numeric(quantile(x_v_hat, 0.75, na.rm = TRUE)),
    Max     = max(x_v_hat, na.rm = TRUE)
  )
}
...
The two panels below show the same data: a very weak positive relationship between urbanization and murder arrests across 50 American states.
The left panel piles on non-data ink; the right strips it back.
Both panels carry the same information about the data.
```{r}
par(mfrow=c(1, 2), mar=c(4, 4, 2, 1))

# Cluttered: oversized two-color markers, gridlines, redundant title
plot(y_hat ~ x_hat, data=xy_hat,
...
```
Removed: oversized two-color markers, grid, box, redundant title, subtitle, one-entry legend.
...
:::{.callout-note icon=false collapse="true" title="Must Know"}
**Polishing a plot with many points.** The simulated data below have 45,001 points, so a default first draft would be a solid black blob.
The second draft standardizes both variables, uses small transparent points, and adds a 45 degree line for reference.
```{r}
# Random Data
x_sim <- seq(1, 10, by=.0002)
e <- rnorm(length(x_sim), mean=0, sd=1)
y_sim <- .25*x_sim + e
...
x_std_sim <- scale(x_sim)
y_std_sim <- scale(y_sim)
plot(x_std_sim, y_std_sim,
    xlab='', ylab='',
...
The next version of the same data has good features, but goes too far.
The math in the axis labels, the outer legend, and the long title all add ink the reader does not need.
```{r}
# Another Example
xy_sim <- data.frame(x_sim=x_sim, y_sim=y_sim)
par(fig=c(0, 1, 0, 0.9), new=FALSE)
plot(y_sim ~ x_sim, xy_sim, pch=16, col=grey(0, .05), cex=.5,
    main=NA, xlab='', ylab='') # Format Axis Labels Separately
mtext( 'y=0.25 x + e\n e ~ standard-normal', 2, line=2.2)
mtext( expression(x%in%~'[0, 10]'), 1, line=2.2)
#abline( lm(y_sim ~ x_sim, data=xy_sim), lty=2)
...
y_small_hat <- xy_hat[1:6, 'y_hat']
stripchart(y_small_hat, pch=16, col=grey(0, .6),
    xlab='Murder Arrests')
```
...
Sometimes tables are better than graphs (see <https://www.edwardtufte.com/notebook/boxplots-data-test>).
...
* *Data Analysis:* 
    * Make a simple scatterplot for `Murder` vs. `UrbanPop` in the `USArrests` data
