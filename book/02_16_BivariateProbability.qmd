...
:::{.callout-important title="Definition" collapse="false"}
The *joint probability* $Prob(X_{i}=x, Y_{i}=y)$ is the probability that both events $X_{i}=x$ and $Y_{i}=y$ occur. The joint distribution is the table of these probabilities over all pairs $(x,y)$, summing to one.
:::
...
:::{.callout-important title="Definition" collapse="false"}
Two random variables are *statistically independent* when the joint probability factors as the product of the marginals: $Prob(X_{i}=x, Y_{i}=y)=Prob(X_{i}=x)Prob(Y_{i}=y)$ for every $(x,y)$. Equivalently, knowing $Y_{i}$ tells you nothing about the distribution of $X_{i}$.
:::
...
:::{.callout-important title="Definition" collapse="false"}
A *bivariate normal distribution* describes a random vector $(X_{i}, Y_{i})$ whose joint density is the multivariate normal with mean vector $(\mu_{X}, \mu_{Y})$, variances $(\sigma_{X}^{2}, \sigma_{Y}^{2})$, and correlation $\rho$. Both marginals are normal, and every conditional $Y_{i} \mid X_{i}=x$ is also normal with a linear conditional mean.
:::
...
:::{.callout-important title="Definition" collapse="false"}
The *conditional expectation* $\mathbb{E}[Y_{i}|X_{i}=x]=\sum_{y} y\,Prob(Y_{i}=y\mid X_{i}=x)$ is the population mean of $Y_{i}$ inside the subpopulation where $X_{i}=x$. It is the theoretical version of the sample conditional mean $\hat{M}_{Y|x}$ from [Two Groups](https://jadamso.github.io/Rbooks/02_11_TwoGroups.html).
:::
...
Similarly, we can compute the theoretical *regression coefficient*: the covariance divided by the variance of $X_{i}$,
\begin{eqnarray}
\beta_{1} = \frac{\mathbb{C}[X_{i}, Y_{i}] }{ \mathbb{V}[X_{i}] }.
\end{eqnarray}
This is the population counterpart of the sample slope $\hat{b}_{1} = \hat{C}_{XY}/\hat{V}_{X}$ from [Simple Regression](https://jadamso.github.io/Rbooks/02_13_SimpleRegression.html).

:::{.callout-important title="Definition" collapse="false"}
The *(theoretical) regression coefficient* $\beta_{1}=\mathbb{C}[X_{i}, Y_{i}]/\mathbb{V}[X_{i}]$ is the slope of the best straight-line approximation to the conditional expectation in the population. The sample slope $\hat{b}_{1}$ is what an OLS regression estimates of $\beta_{1}$.
:::
...
\end{eqnarray}
This is the slope of the best straight-line fit. It is positive, even though the conditional expectation computed earlier is non-monotonic (it falls from $x=0$ to $x=1$, then rises). The line captures the overall upward tendency but misses the dip.
```{r}
