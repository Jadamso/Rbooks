...
Are the fair coins statistically independent? Are the unfair coins?
:::
...
prob_table[,'probabilities'] <- as.vector(t(xy_probs))
...
```{r}
# Compute Marginals
pX <- colSums(xy_probs) #Prob(X_i=x)
pY <- rowSums(xy_probs) #Prob(Y_i=y)

# Compute Conditionals 
PY_x <- t( t(xy_probs) / pX ) #Prob(Y_i=y | X_i=x)
PX_y <- xy_probs / pY  #Prob(X_i=x | Y_i=y)
