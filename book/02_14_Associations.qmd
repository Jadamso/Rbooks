...
#plot(xy_hat, pch=16, col=grey(0, .25))
...
:::{.callout-tip icon=false collapse="true" title="Test Yourself"}
```{r}
# xy_hat and xy_codev defined above
...
KT <- cor(xy_rank_hat[, 'x_hat'], xy_rank_hat[, 'y_hat'], method = 'kendall')
round(KT, 3)
```
...
KT <- cor(xy_hat[, 'x_hat'], xy_hat[, 'y_hat'], method='kendall')
```
:::
...
MvNormal <- function(n = 1000, cor, f) {
   for (i in cor) {
      ...
      xy_sim <- mvtnorm::rmvnorm(n, c(0, 0), sd)
      xy_sim[, 2] <- f(xy_sim[, 2])
      MyPlot(xy_sim)
   }
}
...
# DescTools::CramerV( table(xy_bin_hat) )
```
