...
Given a standard error $SE(M)$ and quantiles $q(\alpha/2)$, we know from theory that $E=q(\alpha)\times SE(M)$


The critical values, $M \pm q(\alpha) SE(M)$, define the Normal confidence interval associated with $1-\alpha$ coverage.
...
\begin{eqnarray}
F^{-1}_{Normal}\left(0.975, SE(M) \right) &=& +1.96 \times SE(M) \\
F^{-1}_{Normal}\left(0.025, SE(M) \right) &=& -1.96 \times SE(M)
\end{eqnarray}
Together, we can calculate
\begin{eqnarray}
E &=& q(\alpha) \times SE(M) = 1.96 \times 5 = 9.8\\
