...
We typically have only one sample and compute a single confidence interval with the data we have. Once doing so, we say that "we are $Z\%$ confident that the mean is in this interval". The interval you computed either contains the true mean or it does not.^[As such, a $Z\%$ confidence interval does not imply a $Z\%$ probability that the true parameter lies within a particular calculated interval. In practice, people often interpret confidence intervals informally as "showing the uncertainty around our estimate": wider intervals correspond to higher sampling variability and less precise information about $\mu$. ] So "$Z\%$ confident" translates to "$Z\%$ of intervals constructed on different samples will contain $\mu$".

Just as with standard errors, we can estimate confidence intervals using theory-driven or data-driven approaches. We will focus on data-driven approaches first, which can be easily extended to other statistics. For example, a $Z\%$ confidence interval for the median is an interval where $Z\%$ of those generated will contain the population median. 


#### **Computation**. {-}

We construct a $90\%$ confidence interval by taking the $5^{th}$ and $95^{th}$ percentiles of the sampling distribution. For example, consider the mean of a uniform random sample with a sample size of $n=1000$. We first conduct a simulation that shows the sampling distribution of the sample mean, and then construct a confidence interval using quantiles.
```{r}
...
The quantiles associated with $Z\%$ are called the "critical values" 
