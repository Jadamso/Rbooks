...
A regression line goes through the cloud of points, and the gap between each point and the line is what we need to define first.
...
A *residual* $e_{i}$ is the vertical gap between observation $i$'s actual value of $\hat{Y}_{i}$ and the model's prediction for that observation:
$$e_{i}=\hat{Y}_{i}-(b_{0}+b_{1}\hat{X}_{i}).$$
:::

The *residual* is useful as the basic raw material of every later regression quantity: the OLS coefficients, the $\hat{R}^{2}$, the standard errors, the diagnostic plots are all functions of $e_{i}$. We will square these gaps and sum them, which prevents positive and negative misses from cancelling and penalizes big misses more than small ones.

We then find the parameters which best-fit the data, by minimizing the sum of squared residuals:
\begin{eqnarray}
\min_{b_{0}, b_{1}} \sum_{i=1}^{n} \left( e_{i} \right)^2 &=& \min_{b_{0}, b_{1}} \sum_{i=1}^{n} \left( \hat{Y}_{i} - [b_{0}+b_{1} \hat{X}_{i}] \right)^2.
\end{eqnarray}
This *ordinary least squares* (OLS) problem has a closed-form solution.

:::{.callout-important title="Key Definition" collapse="false"}
The *least-squares* (OLS) coefficients minimize the sum of squared residuals. The optimal slope and intercept are
...
To derive these, take partial derivatives of the objective with respect to each coefficient and set to zero. For the slope:
\begin{eqnarray}
...
:::{.callout-important title="Key Definition" collapse="false"}
A *fitted value* is the regression line evaluated at the $i$-th observation's $\hat{X}_{i}$:
$$\hat{y}_{i} = \hat{b}_{0}+\hat{b}_{1}\hat{X}_{i}.$$
A *prediction* $\hat{y}(x)$ uses the same line at any new design point $x$:
$$\hat{y}(x) = \hat{b}_{0} + \hat{b}_{1} x.$$
The *sample residual* is the gap between actual and fitted, $\hat{e}_{i} = \hat{Y}_{i} - \hat{y}_{i}$.
:::

Fitted values are useful for *in-sample* diagnostics: comparing $\hat{y}_{i}$ to $\hat{Y}_{i}$ shows where the model fits well and where it misses (these are the residuals $\hat{e}_{i}$ we minimized). Predictions $\hat{y}(x)$ are useful for *out-of-sample* questions like "what wage would the model predict for someone with 14 years of schooling?". Extrapolating well beyond the range of $\hat{X}_{i}$ in the sample is risky: the model has no data there to anchor its prediction.
...
:::{.callout-important title="Key Definition" collapse="false"}
The *R-squared* (or *coefficient of determination*) $\hat{R}_{yY}^{2}$ is the share of total variation in $\hat{Y}_{i}$ that the model explains. Using the Total, Explained, and Residual sums of squares,
...
"No association" is the most common null hypothesis in practice, but we can also use bootstrapping to test against other specific hypothesis: $\beta$, the population slope (formalized in [Bivariate Probability](https://jadamso.github.io/Rbooks/02_16_BivariateProbability.html)). To impose the null in this case, you recenter the sampling distribution around the hypothetical value; $\hat{t} = \frac{\hat{b} - \beta}{\hat{s}_{\hat{b}}}$.^[Under some additional assumptions, the null distribution follows a $t$-distribution. (For more on parametric t-testing based on statistical theory, see <https://www.econometrics-with-r.org/4-lrwor.html>.)]
...
It is true that linear regression "is the best linear predictor of the nonlinear regression function if the mean-squared error is used as the loss function" (Cameron and Trivedi 2005, p. 92). But this is not a carte-blanche justification for OLS, as the best of the bad predictors is still a bad predictor. For many economic applications, it is more helpful to think and speak of "dose response curves" instead of "the effect".
...
1. Explain in words what the $R^2$ statistic measures. A researcher runs a regression and finds $\hat{R}^2 = 0.95$. Does this mean the model is correctly specified or that $X$ causes $Y$? Why or why not?

2. Suppose you have $n=4$ observations: $\{(1, 3),\ (2, 5),\ (3, 6),\ (4, 10)\}$. Compute $\hat{b}_0$ and $\hat{b}_1$ by hand using $\hat{b}_1 = \hat{C}_{XY}/\hat{V}_X$ and $\hat{b}_0 = \hat{M}_Y - \hat{b}_1 \hat{M}_X$. Then compute the fitted values $\hat{y}_i$, the residuals $\hat{e}_i$, and $\hat{R}^2$.

3. Using the `mtcars` dataset, regress `mpg` on `wt` with `lm()`. Report $\hat{b}_0$, $\hat{b}_1$, and $\hat{R}^2$. Then write a bootstrap with $B = 999$ iterations to construct a $95\%$ percentile confidence interval for $\hat{b}_1$ and check whether it contains zero.
