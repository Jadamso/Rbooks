...
A *residual* $e_{i}=\hat{Y}_{i}-(b_{0}+b_{1}\hat{X}_{i})$ is the vertical gap between observation $i$'s actual value of $\hat{Y}_{i}$ and the model's prediction for that observation. The OLS objective squares and sums these gaps.
:::


We then find the parameters which best-fit the data. Specifically, our *objective* is to
\begin{eqnarray}
\min_{b_{0}, b_{1}} \sum_{i=1}^{n} \left( e_{i} \right)^2 &=& \min_{b_{0}, b_{1}} \sum_{i=1}^{n} \left( \hat{Y}_{i} - [b_{0}+b_{1} \hat{X}_{i}] \right)^2.
\end{eqnarray}
Minimizing the sum of squared errors then yields two *optimal coefficients* that can be explicitly solved for using math. For the *slope* coefficient:
\begin{eqnarray}
...
\end{eqnarray}
the latter term being the estimated covariance between $X$ and $Y$ divided by the variance of $X$. (Recall that $M_{X}$ and $M_{Y}$ are the means.) Equivalently, $\hat{b}_{1} = \hat{R}_{XY} \cdot \hat{S}_{Y}/\hat{S}_{X}$, so the slope is just the correlation rescaled by the ratio of standard deviations.

:::{.callout-important title="Definition" collapse="false"}
The *least-squares slope* $\hat{b}_{1}=\hat{C}_{XY}/\hat{V}_{X}$ is the value of $b_{1}$ that minimizes the sum of squared residuals when paired with the matching intercept $\hat{b}_{0}=\hat{M}_{Y}-\hat{b}_{1}\hat{M}_{X}$. It is the change in fitted $\hat{Y}$ per one-unit change in $\hat{X}$.
:::
...
Once we have the coefficient, we can find the sample predictions
\begin{eqnarray}
\hat{y}_{i} &=& \hat{b}_{0}+\hat{b}_{1}\hat{X}_{i}\\
\hat{e}_i &=& \hat{Y}_{i}-\hat{y}_{i}
\end{eqnarray}
and make out-of-sample predictions at design points $x$
\begin{eqnarray}
\hat{y}(x) = \hat{b}_0 + \hat{b}_1 x
\end{eqnarray}

:::{.callout-important title="Definition" collapse="false"}
A *fitted value* $\hat{y}_{i}=\hat{b}_{0}+\hat{b}_{1}\hat{X}_{i}$ is the regression line evaluated at the $i$-th observation's $\hat{X}_{i}$. A *prediction* $\hat{y}(x)$ uses the same line at any new design point $x$, including values not in the sample.
:::
...
:::{.callout-important title="Definition" collapse="false"}
The *R-squared* $\hat{R}^{2}_{yY}=\hat{ESS}/\hat{TSS}=1-\hat{RSS}/\hat{TSS}$ is the share of total variation in $\hat{Y}_{i}$ that the model explains. $\hat{R}^{2}=1$ means perfect in-sample fit; $\hat{R}^{2}=0$ means the model does no better than predicting the mean.
:::
...
:::{.callout-important title="Definition" collapse="false"}
The *standard error* of an estimator is the standard deviation of its sampling distribution. For a slope coefficient $\hat{b}_{1}$ we write $\hat{s}_{\hat{b}_{1}}$ for the estimated standard error; it captures how much $\hat{b}_{1}$ would jiggle from sample to sample.
:::
