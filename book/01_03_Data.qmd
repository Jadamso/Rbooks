...
The histogram divides the range of $\{X_{i}\}_{i=1}^{N}$ into $L$ exclusive bins of equal-width $h=[\text{max}(X_{i}) - \text{min}(X_{i})]/L$, and count the number of observations within each bin. We often rescale the counts so that the total area of all bins sums to 1, which allows us to interpret the numbers as a *density estimate*. Mathematically, for an exclusive bin with midpoint $x$, we compute
\begin{eqnarray}
...
Note that if you your data are factor data, or discrete cardinal data, you can directly plot the counts or proportions.
```{r}
xr <- floor(USArrests$Murder) #Discretized data (rounded down)
proportions <- table(xr)/sum(xr)
