...
The loop ran through `i` random datasets before finding one with $p < 0.001$. With truly random data, each regression has a $0.1\%$ chance of producing $p < 0.001$. After $k$ independent tests, the probability of at least one false positive is $1-(1-0.001)^k$. After $k=700$ tries, this exceeds $50\%$. The more you search, the more likely you are to find a spurious result.
:::
