...
In all cases, we measure association but not causation.
...
You can conduct hypothesis tests for these statistics using the same procedures we learned for univariate data, [ch.5](01_07_Intervals.qmd).
...
Importantly, we can also impose the null of hypothesis of no association by reshuffling the data.
...
The Falk codeviance is useful for cardinal data with outliers or heavy tails (income, prices, response times), where one extreme observation can move $\hat{C}_{XY}$ by a lot but $\tilde{C}_{XY}$ by very little (the same robustness rationale as for the median, $IQR$, and $\text{MAD}$ from Part 1).^[See <01_03_DescriptiveStatistics.qmd#medians-and-absolute-deviations.>. See also the *Theil-Sen Estimator*, which may be seen as a precursor.]
...
It almost always helps to visual your data first before summarizing it with a statistic.
...
One major issue is statistical significance: sometimes there are relationships in the population that do not show up in samples, or non-relationships that appear in samples.
This should be familiar at this point.

Other major issues pertain to a) real relationships averaging out and b) mechanically inducing relationships with random data.
To be concrete about these issue, I will focus on the most-used correlation statistic and examine
...
We will delay covering these issues until much later.
...
2. Give an example of a dataset where Pearson's correlation $\hat{R}_{XY}$ is close to zero but there is a clear causal relationship between $X$ and $Y$.
   What feature of the relationship causes the correlation to miss it, and which alternative statistic from this chapter might detect it?

3. For the dataset $\{(1, 5),\ (2, 3),\ (3, 4),\ (4, 2),\ (5, 1)\}$, compute the Pearson correlation $\hat{R}_{XY}$ by hand.
...
4. Using the `USArrests` dataset, compute both the Pearson correlation and Kendall's rank correlation between `Murder` and `Assault`.
...
We closed with simulations showing that association is not causation.
The next chapter takes the cardinal case and fits a line, which moves from "how strongly do $X$ and $Y$ move together" to "by how much does $Y$ change per unit of $X$".
