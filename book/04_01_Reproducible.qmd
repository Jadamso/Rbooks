...
for(p in c(-.5, 0)){
    ...
    hist(x, breaks=50, border=NA, main=NA, freq=FALSE)
}
...
for(p in c(-1, 0, 2)){
    x <- rtlambda(2000, p)
    hist(x, breaks=100, border=NA, main=NA, freq=FALSE)
}
...
for (i in seq_along(bin_levels)) {
    bin_means[i] <- mean(x[x_bins == bin_levels[i]])
}
bin_means

# shortcut
aggregate(x, list(x_bins), mean)
```
