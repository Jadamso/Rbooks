...
par(mfrow=c(1, 2))
for(p in c(-.5, 0)){
    x_hat <- rgev(2000, mu=0, sigma=1, xi=p)
    hist(x_hat, breaks=50, border=NA, main=NA, freq=FALSE)
}
...
par(mfrow=c(1, 3))
for(p in c(-1, 0, 2)){
    x_hat <- rtlambda(2000, p)
    hist(x_hat, breaks=100, border=NA, main=NA, freq=FALSE)
}
...
make_noisy_data <- function(n, b=0){
    ...
    x_hat <- seq(1, 10, length.out=n)
    e <- rnorm(n, mean=0, sd=10)
    y_hat <- b*x_hat + e
    # Obervations
    xy_mat <- data.frame(ID=seq(x_hat), x=x_hat, y=y_hat)
    return(xy_mat)
}
...
for (i in seq_along(melt_vars)) {
    dat_melted2[i, 'DF'] <- melt_vars[[i]][1]
    dat_melted2[i, 'variable'] <- melt_vars[[i]][2]
}
...
for (i in seq_along(bin_levels)) {
    bin_means[i] <- mean(x_hat[x_bins == bin_levels[i]])
}
bin_means

# shortcut
aggregate(x_hat, list(x_bins), mean)
```
...
y_hat <- runif(n)
z_hat <- runif(n)
XYZ <- cbind(x_hat, y_hat, z_hat)

# Inspect 4 equivalent `row mean` calculations 
profvis::profvis({
    m_hat <- rowSums(XYZ)/ncol(XYZ)
    m_hat <- rowMeans(XYZ)
    m_hat <- apply(XYZ, 1, mean)
    m_hat <- rep(NA, n);  for(i in 1:n){ m_hat[i] <- (x_hat[i] + y_hat[i] + z_hat[i]) / 3 }
})
```
...
##  mean1(x_hat, 0.5) 19.99996 21.08001 21.74110 22.06529 22.31428 23.07532    20   a
##  mean2(x_hat, 0.5) 19.15722 20.10658 20.91981 21.12252 21.73134 22.79755    20   a
##  mean3(x_hat, 0.5) 19.94863 20.39546 22.13567 22.02917 22.52014 32.40659    20   a
...
## 1 mean1(x_hat, 0.5)   19.9ms   22.6ms      45.9    7.63MB     0
## 2 mean2(x_hat, 0.5)   19.1ms   21.6ms      48.3    7.63MB     2.54
## 3 mean3(x_hat, 0.5)   19.8ms   22.6ms      45.3    7.63MB     2.39
```
...
all.equal(ma1(x_hat), ma2(x_hat))
## [1] TRUE

system.time( ma1(x_hat) )
##    user  system elapsed 
##   0.173   0.000   0.174
system.time( ma2(x_hat) )
##    user  system elapsed 
##   0.031   0.006   0.037

ma3 <- compiler::cmpfun(ma2)
system.time( ma3(x_hat) )
##    user  system elapsed 
##   0.028   0.000   0.028
```
...
Rprof( interval = 0.005)
    ...
    # Loop Format Data
    z_full_hat <- y_hat*NA
    for(i in 2:length(y_hat)){ z_full_hat[i] <- (y_hat[i]-y_hat[i-1])/2 }
    # Regression
    X <- cbind(1, x_hat)[-1, ]
    z_hat <- z_full_hat[-1]
    reg_fast <- .lm.fit(X, z_hat)
Rprof(NULL)
summaryRprof()
```
...
```{r}
X <- cbind(1, runif(1e6))
y_hat <- X %*% c(1, 2) + rnorm(1e6)
DAT <- as.data.frame(cbind(y_hat, X))

system.time({ lm(y_hat ~ X, data=DAT) })
##    user  system elapsed 
##   0.324   0.007   0.134
system.time({ .lm.fit(X, y_hat) })
##    user  system elapsed 
##   0.093   0.000   0.026
system.time({ solve(t(X)%*%X) %*% (t(X)%*%y_hat) })
##    user  system elapsed 
##   0.029   0.000   0.024
```
