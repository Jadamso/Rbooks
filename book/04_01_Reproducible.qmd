...
Note that there is a difference between replicable and reproducible.
...
First, revist <00_01_FirstSteps.qmd#why-program-in-r>. Then consider the counterfactual below. Future economists should also read <https://core.ac.uk/download/pdf/300464894.pdf>.
...
* You found out Excel had some bugs in it's statistical calculations. You now use a new version of the spreadsheet.
...
You have already accessed "Wages1" dataset from the "Ecdat" package. This package also contains datasets on crime. To use this data, we must first install the package on our computer (which we already did). Then, to use the datasets, we load the package.
...
par(mfrow=c(1, 2))
for(p in c(-.5, 0)){
    x_sim <- rgev(2000, mu=0, sigma=1, xi=p)
    hist(x_sim, breaks=50, border=NA, main=NA, freq=FALSE)
}
title('GEV densities', outer=TRUE, line=-1)
```
...
par(mfrow=c(1, 3))
for(p in c(-1, 0, 2)){
    x_sim <- rtlambda(2000, p)
    hist(x_sim, breaks=100, border=NA, main=NA, freq=FALSE)
}
title('Tukey-Lambda densities', outer=TRUE, line=-1)
```
...
# download source 'https://pages.stern.nyu.edu/ ~ wgreene/Text/Edition7/TableF19-3.csv'
# download destination ' ~ /TableF19-3.csv'
read.csv(' ~ /TableF19-3.csv')
 
# Can read in csv (directly from online)
# dat_csv <- read.csv('https://pages.stern.nyu.edu/ ~ wgreene/Text/Edition7/TableF19-3.csv')
```
...
#library(haven)
#dat_stata <- read_dta('https://users.ssc.wisc.edu/ ~ bhansen/econometrics/DS2004.dta')
#dat_stata <- as.data.frame(dat_stata)

# For More Introductory Econometrics Data, see 
# https://users.ssc.wisc.edu/ ~ bhansen/econometrics/Econometrics%20Data.zip
...
make_noisy_data <- function(n, b=0){
    ...
    x_sim <- seq(1, 10, length.out=n)
    e <- rnorm(n, mean=0, sd=10)
    y_sim <- b*x_sim + e
    # Obervations
    xy_sim <- data.frame(ID=seq(x_sim), x=x_sim, y=y_sim)
    return(xy_sim)
}

# Two simulated datasets
xy1_sim <- make_noisy_data(6)
xy2_sim <- make_noisy_data(6)
...
xy_wide_sim <- cbind( xy1_sim, xy2_sim)
names(xy_wide_sim) <- c(paste0(names(xy1_sim), '.1'), paste0(names(xy2_sim), '.2'))

# Merging data in wide format, Second Attempt
# higher performance
xy_wide2_sim <- merge(xy1_sim, xy2_sim,
    by='ID', suffixes=c('.1', '.2'))
## CHECK they are the same.
identical(xy_wide_sim, xy_wide2_sim)
...
library(data.table)
xy_long_dt_sim <- as.data.table(xy_long_sim)
xy_melt_sim <- melt(xy_long_dt_sim, id.vars=c('ID', 'DF'))
xy_wide3_sim <- dcast(xy_melt_sim, ID ~ DF+variable)

## CHECK they are the same.
identical(xy_wide_sim, xy_wide3_sim)
```

Often, however, we ultimately want data in long format
```{r}
# Merging data in long format, Second Attempt with dedicated package 
xy_melt2_sim <- melt(xy_wide3_sim, measure=c('1_x', '1_y', '2_x', '2_y'))
melt_vars <- strsplit(as.character(xy_melt2_sim[['variable']]), '_')
for (i in seq_along(melt_vars)) {
    xy_melt2_sim[i, 'DF'] <- melt_vars[[i]][1]
    xy_melt2_sim[i, 'variable'] <- melt_vars[[i]][2]
}
xy_long2_sim <- dcast(xy_melt2_sim, DF+ID ~ variable)
xy_long2_sim <- as.data.frame(xy_long2_sim)

## CHECK they are the same.
identical( xy_long2_sim, xy_long_sim)

# Further Inspect
xy_long2_sim <- xy_long2_sim[, c('ID', 'x', 'y', 'DF')]
mapply( identical, xy_long2_sim, xy_long_sim)
```
...
bin_levels <- levels(x_bins)
bin_means <- numeric(length(bin_levels))
names(bin_means) <- bin_levels
for (i in seq_along(bin_levels)) {
    bin_means[i] <- mean(x_toy[x_bins == bin_levels[i]])
}
bin_means

# shortcut
aggregate(x_toy, list(x_bins), mean)
```
...
**Poster**. 
See [DataScientism_Poster.html](https://github.com/Jadamso/Rbooks/blob/main/Templates/Presentations/DataScientism_Poster.html) and recreate from the source file [DataScientism_Poster.Rmd](https://github.com/Jadamso/Rbooks/blob/main/Templates/Presentations/DataScientism_Poster.Rmd). Simply change the name to your own, and knit the document.

**Slides**.
See [DataScientism_Slides.pdf](https://github.com/Jadamso/Rbooks/blob/main/Templates/Presentations/DataScientism_Slides.pdf) and recreate from the source file [DataScientism_Slides.Rmd](https://github.com/Jadamso/Rbooks/blob/main/Templates/Presentations/DataScientism_Slides.Rmd).
...
**Example: Histogram**.
Download the source file [TrialApp1_Histogram_Dashboard.Rmd](https://github.com/Jadamso/Rbooks/blob/main/Templates/Apps/TrialApp1_Histogram_Dashboard.Rmd)
...
If you are having difficulty, you can try working first with the barebones shiny code. To do this, download [TrialApp0_Histogram.Rmd](https://github.com/Jadamso/Rbooks/blob/main/Templates/Apps/TrialApp0_Histogram.Rmd) and edit it in Rstudio. You can run the code with `rmarkdown::run('TrialApp0_Histogram.Rmd')`.
...
<!--# Ignore warnings/messages
    ...
    try(1+2, silent=TRUE)
    ## [1] 3
    try(warning('warning'), silent=TRUE)
    try(error('error'), silent=TRUE)
    
    try(1+2, silent=FALSE)
    ## [1] 3
    try(warning('warning'), silent=FALSE)
    try(error('error'), silent=FALSE)
...
y_sim <- runif(n)
z_sim <- runif(n)
xyz_sim <- cbind(x_sim, y_sim, z_sim)

# Inspect 4 equivalent `row mean` calculations 
profvis::profvis({
    m_hat <- rowSums(xyz_sim)/ncol(xyz_sim)
    m_hat <- rowMeans(xyz_sim)
    m_hat <- apply(xyz_sim, 1, mean)
    m_hat <- rep(NA, n);  for(i in 1:n){ m_hat[i] <- (x_sim[i] + y_sim[i] + z_sim[i]) / 3 }
})
```
...
mean1 <- function(x_hat, p=1) mean(x_hat^p)
mean2 <- function(x_hat, p=1) sum(x_hat^p) / length(x_hat)
mean3 <- function(x_hat, p=1) mean.default(x_hat^p)
...
##  mean1(x_sim, 0.5) 19.99996 21.08001 21.74110 22.06529 22.31428 23.07532    20   a
##  mean2(x_sim, 0.5) 19.15722 20.10658 20.91981 21.12252 21.73134 22.79755    20   a
##  mean3(x_sim, 0.5) 19.94863 20.39546 22.13567 22.02917 22.52014 32.40659    20   a
...
## 1 mean1(x_sim, 0.5)   19.9ms   22.6ms      45.9    7.63MB     0
## 2 mean2(x_sim, 0.5)   19.1ms   21.6ms      48.3    7.63MB     2.54
## 3 mean3(x_sim, 0.5)   19.8ms   22.6ms      45.3    7.63MB     2.39
```
...
ma1 <- function(y_hat){
    z_hat <- y_hat*NA
    for(i in 2:length(y_hat)){
        z_hat[i] <- (y_hat[i]-y_hat[i-1])/2
    }
    return(z_hat)
}
...
all.equal(ma1(x_sim), ma2(x_sim))
## [1] TRUE

system.time( ma1(x_sim) )
##    user  system elapsed 
##   0.173   0.000   0.174
system.time( ma2(x_sim) )
##    user  system elapsed 
##   0.031   0.006   0.037

ma3 <- compiler::cmpfun(ma2)
system.time( ma3(x_sim) )
##    user  system elapsed 
##   0.028   0.000   0.028
```
...
Rprof( interval = 0.005)
    ...
    # Loop Format Data
    z_full_sim <- y_sim*NA
    for(i in 2:length(y_sim)){ z_full_sim[i] <- (y_sim[i]-y_sim[i-1])/2 }
    # Regression
    X <- cbind(1, x_sim)[-1, ]
    z_sim <- z_full_sim[-1]
    reg_fast <- .lm.fit(X, z_sim)
Rprof(NULL)
summaryRprof()
```
...
```{r}
X <- cbind(1, runif(1e6))
y_sim <- X %*% c(1, 2) + rnorm(1e6)
xy_sim <- as.data.frame(cbind(y_sim, X))

system.time({ lm(y_sim ~ X, data=xy_sim) })
##    user  system elapsed 
##   0.324   0.007   0.134
system.time({ .lm.fit(X, y_sim) })
##    user  system elapsed 
##   0.093   0.000   0.026
system.time({ solve(t(X)%*%X) %*% (t(X)%*%y_sim) })
##    user  system elapsed 
##   0.029   0.000   0.024
```
