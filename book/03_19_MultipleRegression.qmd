...
We can analytically solve this problem using [matrix-calculations](https://jadamso.github.io/Rbooks/03_26_MiscTopics.html#matrix-calculations), although we do not do that here. Solving yields the best fitting model with coefficients $B^{*}=(b_0^{*} ~~ b_1^{*} ~~... ~~ b_{K}^{*})$.
...
R2_sim <- vector(length=length(ksims))
R2adj_sim <- vector(length=length(ksims))
for(k in ksims){
...
boots <- 1:399
R2adj_sim0 <- vector(length=length(boots))
for(b in boots){
