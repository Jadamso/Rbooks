...
A *multivariate regressogram* cuts each explanatory variable into exclusive bins and predicts the mean of $\hat{Y}$ within each bin combination. It is the multivariate generalization of the univariate regressogram from [Local Regression](https://jadamso.github.io/Rbooks/02_14_LocalRegression.html#regressograms).
:::
...
A regressogram is *locally constant*: it predicts a single number for every point in a bin.
Just as in the bivariate case, we can instead fit a *locally linear* model by running a separate regression within each bin.
This is a multivariate *piecewise regression*: we bin one variable, then fit a full linear model of $\hat{Y}$ on the explanatory variables within each bin.

:::{.callout-important title="Definition" collapse="false"}
A multivariate *piecewise regression* bins one explanatory variable and fits a separate OLS line on the others within each bin. Predictions vary smoothly within a bin and jump only at the bin edges, sitting between the flat regressogram and the global OLS plane.
:::
...
One of the most common approaches to selecting a model or bandwidth is to minimize *prediction* error. *Leave-one-out cross-validation* minimizes the average "leave-one-out" mean square prediction errors:
\begin{eqnarray}
...
:::{.callout-important title="Definition" collapse="false"}
*Cross-validation (CV)* picks a model by minimizing out-of-sample prediction error. *Leave-one-out CV* drops one observation, refits, and averages the squared prediction errors; *$k$-fold CV* drops one of $k$ random groups at a time. CV penalizes overfitting because too-flexible models predict the held-out observations poorly.
:::
...
We first summarize gradients (how $Y$ changes with $X$) before testing them. There are two main approaches.
...
:::{.callout-important title="Definition" collapse="false"}
A *gradient* of a fitted multivariate model is the partial derivative $\partial \hat{y}/\partial x_p$, evaluated at a point. A regressogram has no slope coefficients, so its gradient must be approximated by finite differences; a piecewise or local linear regression has within-bin slopes that are themselves gradient estimates.
:::
...
This chapter generalized regressograms, piecewise regression, and cross-validation to the multivariate setting, and showed two ways to summarize gradients: finite differences from the regressogram, and within-bin slopes from the piecewise model. In the next chapter we step back from estimation and ask what happens when the data themselves are dependent over time, across space, or through market equilibrium — observational complications that violate the independence assumption used here.
:::
