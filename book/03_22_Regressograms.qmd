...
The predictions now vary within each bin and jump only at the bin edges, sitting between the flat regressogram and the global OLS plane.
...
A fully *local linear regression* generalizes this further, replacing hard bins with kernel weights and a separate bandwidth for each variable (see Model Selection below).
...
There are many types of cross-validation (Arlot and Celisse, 2010; Bates et al., 2023). For example, one extension is *k-fold cross-validation*, which splits the $n$ datapoints into $K$ groups, fits the model $K$ times (each time leaving one group out), and predicts the values in the left-out group. *Generalized cross-validation* adjusts for the degrees of freedom, whereas the `npreg` function in R uses *least-squares cross-validation* (Racine, 2019, p. 74) by default. You can refer to extensions on a case by case basis.
