...
For another example, consider a bivariate random vector where there are three outcomes for $X_{i}$ and three outcomes for $Y_{i}$. The plot below shows higher probability events are reflected with deeper colors. 
```{r}
