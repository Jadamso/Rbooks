...
:::{.callout-tip icon=false collapse="false" title="Test Yourself"}
Provide a concrete example of computing a weighted variance building on this code below
```{r, eval=F}
mx <- sum(x*x_probs)
x_diff <- (x - mx)^2
x_var <- sum(x_probs * x_diff)
```
:::
...
Note that the standard deviation refers variability of individual observations (in either a sample or population), and is hence different from the standard error. To be clear, 
