...
Often, we are interested in *gradients*: how $Y$ changes with $X$. The linear model from [Simple Regression](02_13_SimpleRegression.qmd) depicts this as a single constant, $\hat{b}_{1}$, but the local-regression models from [Local Regression](02_14_LocalRegression.qmd) do not. A great first step to assess gradients is to plot the predicted values over the explanatory values. A great second step is to compute gradients and summarize them. In any case, we compute confidence intervals to account for variability across samples.
