...
The bias-variance tradeoff is useful for thinking about *why* a tuning parameter matters and how to set it. For local regressions, the bandwidth $h$ (equivalently, the number of bins) is the lever that governs the tradeoff: large $h$ trades variance down for bias up, small $h$ does the opposite. A regressogram with one bin per observation has $\hat{MSE}=0$ on the fit data but generalizes poorly because it has just copied the noise.
