...
*LOESS* (locally estimated scatterplot smoothing) fits a local linear (or quadratic) regression at each design point, using an *adaptive bandwidth* that includes a fixed share of the data. The share is set by `span`; smaller spans mean narrower neighborhoods, more flexibility, and noisier curves.
:::
...
:::{.callout-important title="Definition" collapse="false"}
A *confidence band* is a confidence interval drawn at every design point: a region around the fitted curve that captures sample-to-sample variability. A bootstrap confidence band collects the predictions from many resamples and reads off pointwise quantiles.
:::
...
:::{.callout-important title="Definition" collapse="false"}
A *gradient* $\hat{b}_{1}(x)$ at the design point $x$ is the rate of change of the fitted value $\hat{y}(x)$ with respect to $\hat{X}_{i}$ in a neighborhood of $x$. For a globally linear model it equals the single slope coefficient; for local models it varies with $x$.
:::
...
:::{.callout-important title="Definition" collapse="false"}
The *marginal effect at the mean* evaluates the gradient at a single design point (usually $\hat{M}_{X}$) and reports one number. The *mean of the gradients* (also called the *average effect*) averages $\hat{b}_{1}(x)$ across every observation in the dataset. The two coincide for a globally linear model and can differ for any other.
:::
...
:::{.callout-important title="Definition" collapse="false"}
The expected prediction error of an estimator decomposes into two pieces. Squared *bias* is how far its average prediction sits from the truth. *Variance* is how much its predictions wobble around their own average. Cutting one usually raises the other.
:::
...
This chapter put inference around the local fits from the previous chapter: bootstrap confidence bands, gradient summaries (marginal effect at the mean vs mean of the gradients), hypothesis tests, and a simulation walk through bias, variance, and consistency. The next chapter pulls back from samples and develops the bivariate probability machinery (joint distributions, conditional expectations, and the population coefficient $\beta_{1}$) that says what we are estimating.
:::
