...
One main way to conduct hypothesis tests is to examine whether a [confidence interval](https://jadamso.github.io/Rbooks/01_07_Intervals.html) contains a hypothesized value. We then use this decision rule
...
A *$p$-value* is the frequency you see something at least as extreme as your statistic under the null hypothesis (when sampling from the null distribution). We want the probability that the random variable $M$ is at least as extreme (far from the null mean of $9$) as our observed sample mean $\hat{M}$.
...
At the $5\%$ level, you expect roughly $1$ in $20$ true nulls to be rejected by chance alone.
If you test $20$ independent nulls and find $1$ "significant" result, that result is what you would expect even when every null is true.
...
This chapter built hypothesis tests two ways (invert a CI, impose the null), introduced $p$-values and $t$-values, and flagged the multiple-comparisons pitfall.
