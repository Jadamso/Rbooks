...
* Bootstrap estimates the distribution of $\hat{M}-M$.
* Null-bootstrap shifts the bootstrap so they are centered at the hypothesized mean, estimating the distribution of $\hat{M}-\mu$.
...
For any specific sample, we must approximate the standard error. Using the theory-driven approach, we compute $\hat{t}=(\hat{M}-\mu)/(\hat{S}/\sqrt{n})$. Using the theory-driven approach, we compute $\hat{t}=(\hat{M}-\mu)/(\hat{SE}^{\text{jack}})$ or $\hat{t}=(\hat{M}-\mu)/(\hat{SE}^{\text{boot}})$. In any case, we can approximate use bootstrapping to estimate the variability of the $t$ statistic, just like we did with the mean.
