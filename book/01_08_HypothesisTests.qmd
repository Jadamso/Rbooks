...
\end{eqnarray}
which corresponds to the estimator $t = (M - \mu) / \mathbb{s}(M)$, which varies from sample to sample. We can use bootstrapping to estimate the variability of the $t$ statistic, just like we did with the mean.
