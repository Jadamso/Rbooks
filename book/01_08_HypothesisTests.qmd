...
The *alternative hypothesis* $H_{A}$ is the competing claim we would accept if the null were rejected.
A *two-sided* alternative ($H_{A}: \mu \neq \mu_{0}$) covers deviations in either direction; a *one-sided* alternative ($H_{A}: \mu > \mu_{0}$ or $H_{A}: \mu < \mu_{0}$) covers only one tail.
:::
...
The most common test concerns the mean, $H_{0}: \mu = \mu_{0}$.
