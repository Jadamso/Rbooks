...
Each bin has a midpoint, $x$, and each observation has an associated dummy variable $\hat{d}_{i}(x,h) = \mathbf{1}\left(\hat{x}_{i} \in \left(x-h,x+h\right] \right)$.
...
It also tunes LOESS in the next chapter, in disguise as `span`.
...
$$\begin{aligned}
\hat{y}_{i} &= \sum_{x \in \{x_{1}, ..., x_{L} \}} b_{0}(x,h) \hat{d}_{i}(x,h)  + \hat{e}_{i},
\end{aligned}$$
...
$$\begin{aligned}
\text{Wage}_{i} &= b_{0}(x=4.5, h=9/2) \mathbf{1}\left(\text{Educ}_{i} \in (0,9]\right) + b_{0}(x=13.5, h=9/2) \mathbf{1}\left(\text{Educ}_{i} \in (9,18] \right) + \hat{e}_{i}.
\end{aligned}$$
...
pred_dat <- data.frame(xcc=dat$xcc, y_fit=round(y_fit_c, 6))
table(pred_dat)
...
y_fit_split <- vector('list', length(dat2))
names(y_fit_split) <- names(dat2)
for (i in seq_along(dat2)) {
    reg2 <- lm(wage ~ school, dat2[[i]])
    y_fit_split[[i]] <- predict(preg_c)
}

# Any differences?
all( y_fit_piecewise - unlist(y_fit_split) < 1e-10 )
```
:::
...
#plot( xy[, 'x_hat'], resid(reg), pch=16, col=grey(0, .5))

# Linear in 2 Pieces (subsets)
xcut2 <- cut(xy[, 'x_hat'], 2)
...
xcut3 <- cut(xy[, 'x_hat'], seq(32, 92, by=20)) # Finer Bins
...
# Compare Predictions
plot(y_hat ~ x_hat, pch=16, col=grey(0, .5), data=xy)
...
\end{aligned}$$
with is nearly identical to $\hat{d}_{i}(x,h)/ 2$, but uses intervals $[]$ instead of $(]$.
...
:::{.callout-note icon=false collapse="true" title="Must Know"}
See how the results are nearly identical below.
Then consider different design points.
```{r}
#library(Ecdat) # Data from before
#dat <- Wages1[order(Wages1[, 'school']), c('wage', 'school')]
...
## Generate Sample Data
x_hat <- 1:5
y_hat <- rnorm(length(x_hat))
y_hat

## Manually Compute Estimate at x=5
h <- 2
k5_weights <- dunif( abs(x_hat-5)/h, -1, 1) #(x_hat >= 3)*(x_hat =< 7)/2
k5_weights 

y_fit_5 <- sum(k5_weights*y_hat)
y_fit_5

## Edge Correction
k5_weights_ec <- k5_weights/sum(k5_weights) 
y_fit_5_ec <- sum(k5_weights_ec*y_hat)
y_fit_5_ec
```
:::
...
pred_fun <- function(x0, h){
    ...
    y_fit <- predict(llls_i, newdata=data.frame(school=x0))
}
...
```{r, eval=FALSE}
xy <- USArrests[, c('Murder', 'UrbanPop')]
x_values <- sort(unique(xy[, 'UrbanPop']))
plot(y_hat ~ x_hat, pch=16, data=xy, col=grey(0, .5),
    ylab='Murder Rate', xlab='Population Density')

y_fit_1 <- numeric(length(x_values))
for (i in seq_along(x_values)) {
    y_fit_1[i] <- pred_fun(x_values[i], h=2, xy=xy)
}

y_fit_2 <- numeric(length(x_values))
for (i in seq_along(x_values)) {
    y_fit_2[i] <- pred_fun(x_values[i], h=20, xy=xy)
}
...
2. Explain the difference between a regressogram and a local linear regression.
   In what sense does increasing the number of bins in a regressogram create a bias-variance tradeoff?
...
The next chapter asks how to evaluate these fits and how much they vary across samples: model-fit metrics ($\hat{MSE}$ and $\hat{MAPE}$), the bias-variance tradeoff, bootstrap confidence bands, gradient summaries, and consistency.
