...
A *prediction interval* for $Y_i$ at $X_i = x$ is a range that captures a stated probability (e.g., 95%) of new observations at that $x$. Unlike a confidence interval for $m(x)$, it also accounts for residual variation, so it is generally wider.
:::
```{r}
...
:::{.callout-important title="Definition" collapse="false"}
*Leave-one-out cross-validation (LOO-CV)* fits the model $n$ times, each time leaving one observation out and predicting it from the rest. The average squared prediction error across the held-out observations is the cross-validation score; the bandwidth or model that minimizes this score is the preferred one.
:::
\begin{eqnarray}
...
:::{.callout-important title="Definition" collapse="false"}
*Filtering* estimates the value of a time series at $t$ using only past observations; *smoothing* uses both past and future. Filtering is appropriate for forecasting and any online use; smoothing produces lower-variance estimates but is only valid for retrospective analysis.
:::
...
:::{.callout-important title="Definition" collapse="false"}
*Bayes' theorem* updates a prior probability $Prob(X_i=x)$ to a posterior $Prob(X_i=x \mid Y_i=y)$ using the likelihood $Prob(Y_i=y \mid X_i=x)$. Mnemonic: posterior $\propto$ likelihood $\times$ prior.
:::
\begin{eqnarray}
...
:::{.callout-important title="Definition" collapse="false"}
The *family-wise error rate (FWER)* is the probability of at least one false positive across a family of $K$ tests. With $\alpha = 0.05$ and independent tests, $\text{FWER} = 1-(1-\alpha)^K$, which exceeds $0.5$ around $K=14$ and approaches $1$ for $K \geq 100$.
:::
...
:::{.callout-important title="Definition" collapse="false"}
The *Bonferroni correction* multiplies each $p$-value by the number of tests $K$ (equivalently, lowers the threshold to $\alpha/K$); it guarantees $\text{FWER} \leq \alpha$ but is conservative. The *Holm correction* applies the same idea stepwise, beating Bonferroni in power while still controlling the FWER.
:::
...
:::{.callout-important title="Definition" collapse="false"}
A *post-hoc test* is a follow-up comparison conducted after a significant global test (like ANOVA). Because there are $G(G-1)/2$ pairs to test, the per-pair significance threshold must be tightened to control the family-wise error rate; *Tukey's HSD* and Holm-adjusted pairwise $t$-tests are common choices.
:::
