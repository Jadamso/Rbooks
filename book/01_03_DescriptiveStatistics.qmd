...
The mean (also called the *sample mean* or *empirical mean*) is the most commonly reported single-number summary of a sample, and the natural one when the data are roughly symmetric. Here $\hat{X}_{i}$ denotes the value of the $i$-th observation.
...
We use the empirical *median* as a "robust alternative" to the mean, especially useful for data with asymmetric distributions or extreme values. Recall from [Data & Visualization](https://jadamso.github.io/Rbooks/01_02_Data.html#quantiles) that the $q$th quantile is the value where $q$ percent of the data fall below and ($1-q$) percent above; the median is the $q=0.5$ case. A single outlier can pull the mean by an arbitrary amount, but moves the median by at most one position, making the median the safer choice whenever extreme values may have crept into the sample.
...
Where skewness uses the cube (and so can be negative), kurtosis uses the fourth power, which forces every term to be positive and disproportionately amplifies the largest deviations. A handful of far-from-the-mean values can drive kurtosis up substantially even when the bulk of the distribution looks unremarkable, which is why it is often described as a "tail-heaviness" measure.
