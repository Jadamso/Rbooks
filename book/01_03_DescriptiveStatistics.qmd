...
A single outlier can pull the *mean* by an arbitrary amount, but moves the median by at most one position; the $IQR$ and $\text{MAD}$ inherit the same protection because they are built from medians and quantiles (recall the [$p$-th quantile](https://jadamso.github.io/Rbooks/01_02_Data.html#quantiles) from the previous chapter).
