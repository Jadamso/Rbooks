...
In each case we summarize the groups, compute a statistic for the difference between them, and test that difference using the resampling methods from [Hypothesis Testing](https://jadamso.github.io/Rbooks/01_08_HypothesisTests.html).
...
Second, we can compute the *conditional mean* $\hat{M}_{Y|X}$ by looking within each row. Specifically
\begin{eqnarray*}
...
The *conditional mean* $\hat{M}_{Y|x}$ is the average value of $Y_{i}$ inside the subgroup where $X_{i}=x$: $\hat{M}_{Y|x}=\sum_{y} y\,\hat{p}_{y\mid x}$. Comparing $\hat{M}_{Y|x}$ across values of $x$ is how we summarize an association between a cardinal outcome and a factor.
:::
...
To compare wages for people with and without completing a degree, we start by making a [histogram](https://jadamso.github.io/Rbooks/01_02_Data.html#histogram-density) for both groups.
...
:::{.callout-important title="Definition" collapse="false"}
The *mean difference* $\hat{D}=\hat{M}_{Y1}-\hat{M}_{Y2}$ is the sample-mean of the outcome in group 1 minus the sample-mean in group 2. A positive value says group 1 has the higher average; zero is the natural null.
:::

The bootstrap below pegs $\hat{M}_{Y1}$ and $\hat{M}_{Y2}$ at the wage data and reports $\hat{D}$. The university group has the higher mean wage, so $\hat{D}<0$ here; the next step is asking whether the gap is bigger than sampling noise alone would produce.
...
For "imposing the null", it is more common to use $p$-values instead of confidence intervals. We can compute $p$-values using a null-bootstrap or *permutation* distribution; a permutation distribution reshuffles the group labels without replacement, so each resample keeps the same pooled data but randomly re-splits it into two groups. If using a hard decision rule, it is most common to use
...
https://jadamso.github.io/Rbooks/01_02_Data.html#empirical-cumulative-distribution-function)'s for both groups.
...
:::{.callout-important title="Definition" collapse="false"}
The *Kolmogorov-Smirnov statistic* $\hat{KS}$ summarizes the difference between two ECDF's with their single biggest vertical gap. It is sensitive to a shift between the two distributions but can miss differences that average out.
:::

:::{.callout-important title="Definition" collapse="false"}
The *Cramer-von Mises statistic* $\hat{CVM}$ summarizes the difference between two ECDF's with the sum (or sum of squares) of vertical gaps at every pooled sample point. It accumulates many small differences that $\hat{KS}$ would miss.
:::
...
These methods extend naturally from two groups to many. See [Multiple Groups](https://jadamso.github.io/Rbooks/03_20_MultipleGroups.html) for the ANOVA approach to comparing $G$ groups simultaneously.
