...
*Control* is the researcher's ability to hold other factors constant; *randomization* is the assignment of treatment in a way that is statistically independent of every pre-treatment variable. Together they break the correlation between a regressor and the error term that produces endogeneity in observational data.
:::
...
The basic idea of *regression discontinuity design* (RDD) and *regression kink design* (RKD) is to examine how a variable changes just before and just after a treatment (for a thorough introduction, see [The Effect, Ch. Regression Discontinuity](https://theeffectbook.net/ch-RegressionDiscontinuity.html)). RDD estimates the difference in the levels of an outcome variable, whereas RKD estimates the difference in the slope. Turning to our canonical competitive market example, the RDD estimate is the difference between the lines at $T=300$.
...
The basic idea of *difference-in-differences* (DID) is to examine how a variable changes in response to an exogenous shock, *compared to a control group* (see [The Effect, Ch. Difference-in-Differences](https://theeffectbook.net/ch-DifferenceinDifference.html) for more detail).
...
Randomization is the process of randomly manipulating an explanatory variable (assigning treatments to units).

A *completely randomized* design is useful when the experimental units are homogeneous: each unit is assigned to treatment or control with equal probability.
If the experimental units are heterogeneous, *blocking* first partitions the units into homogeneous groups and then randomizes *within* each block, ensuring that treatment and control are balanced on the blocking variable.

:::{.callout-important title="Definition" collapse="false"}
*Blocking* partitions experimental units into homogeneous groups before randomly assigning treatment within each group. Blocking buys precision when the blocking variable predicts the outcome, because it removes one source of between-arm variation by construction.
:::
...
Consider the market equilibrium example, which contains a cost shock (for a broader treatment of IV methods, see [The Effect, Ch. Instrumental Variables](https://theeffectbook.net/ch-InstrumentalVariables.html)). We can simply run another regression, but there will still be a problem.
...
1. In the competitive market simulation, a supply cost shock is used to identify the demand slope via 2SLS. Explain why simply regressing $Q$ on $P$ with both cost regimes pooled does not recover the demand slope $-\beta_D$, and what role the exclusion restriction plays in the 2SLS approach.
...
This chapter introduced the experimental and quasi-experimental tools — RDD, DID, blocking, 2SLS — for recovering causal effects from data with limited control. Each method exploits a particular kind of exogenous variation: a cutoff, a treatment timing, an instrument. In the next chapter we caution that the same recipes can produce statistically significant findings from data that contain no causal signal at all.
:::
