...
In general, we use the *[Binomial distribution](https://en.wikipedia.org/wiki/Binomial_distribution)*. The $\binom{n}{x}$ term is called the [binomial coefficient](https://jadamso.github.io/Rbooks/01_05_Sampling.html#sampling-distributions) and it counts all the different groups lead to the same sum. The other terms intuitively reflect the probabilities of getting $x$ successes and $n-x$ failures.  
...
:::{.callout-caution title="Common Pitfall" collapse="false"}
...
The fraction of customers who make a purchase varies from day to day. Some days 80% buy something; other days only 30%. The [Beta distribution](https://en.wikipedia.org/wiki/Beta_distribution) models this kind of proportion: a random variable on the unit interval $X_{i} \in [0,1]$ with flexible shape.
...
This chapter rounded out univariate probability with the Binomial, Poisson, Irwin–Hall, and Beta, plus set theory, Bayes' theorem, and the Law of Total Probability.
