...
This chapter rounds out the probability toolbox with three commonly-encountered families: the Binomial and Poisson (discrete counts), and the Irwin–Hall and Beta (continuous random variables built from Uniforms).
...
In general, we use the *Binomial distribution*. The $\binom{n}{x}$ term is called the [binomial coefficient](https://jadamso.github.io/Rbooks/01_05_Sampling.html#sampling-distributions) and it counts all the different groups that lead to the same sum. The other terms intuitively reflect the probabilities of getting $x$ successes and $n-x$ failures.
...
:::{.callout-important title="Key Definition" collapse="false"}
Two events $A$ and $B$ are *independent* if $Prob(A \cap B) = Prob(A) \cdot Prob(B)$, equivalently if knowing $B$ occurred does not change the probability of $A$.

The *conditional probability* of $A$ given $B$ is $Prob(A \mid B) = Prob(A \cap B) / Prob(B)$ (defined whenever $Prob(B) > 0$).
:::
