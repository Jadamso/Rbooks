...
A *simple random sample* (SRS) is a sample where every subset of size $n$ has the same probability of being chosen.
:::

The SRS is useful as the default baseline: every other sampling scheme (stratified, clustered, weighted) is described by how it departs from it, and the mathematical results in the rest of the chapter assume an SRS.
...
The Law of Large Numbers generalizes to many other statistics, like the `median` or `sd` from [Numerical Statistics](https://jadamso.github.io/Rbooks/01_03_DescriptiveStatistics.html).^[When a statistic converges in probability to the quantity they are meant to estimate, they are called *consistent*.] 
...
hist(Bmeans, xlim=c(-3, 3), ylim=c(0, 1), breaks=61, border=NA,
     axes=F, col='gray', freq=F, main=NA, yaxs='i',
     xlab='', ylab='')
...
for(b in seq_along(bootstrap_meds)){
    dat_b <- sample(small_dat, replace=T)
