...
m3_hat <- mean( population_values[c(1, 4)] ) #{3, 4}
...
for(i in seq_along(sample_sds)){
    x_hat <- runif(100) # same distribution
    s_hat <- sd(x_hat) # different statistic
    sample_sds[i] <- s_hat
}
hist(sample_sds,
...
for(n in c(5, 50, 500)){
    ...
    for(i in seq_along(sample_quants_n)){
        x_hat <- runif(n)
        q_hat <- quantile(x_hat, probs=0.75) #upper quartile
...
for(n in c(5, 50, 500)){
    ...
    for(i in seq_along(sample_props_n)){
        x_hat <- sample(1:5, size=n, prob=c(1:5)/15, replace=TRUE)
        p3_hat <- mean(x_hat==3)
...
PlotSample(x_hat, lineM=m_hat)
axis(1, m_hat, expression(hat(m)), col=rgb(1, 0, 0, .8))

### Bootstrap samples
set.seed(2)  ## Cherry Pick Some Good Ones
boot_samples <- lapply(1:20, function(b) { sample(x_hat, replace=TRUE) })
...
```{r}
x_small_hat <- c(1, 6, 7, 22)
jackknife_meds <- rep(NA, length(x_small_hat))
for(i in seq_along(jackknife_meds)){
    jackknife_meds[i] <- median(x_small_hat[-i])
}
jackknife_meds
mean(jackknife_meds)
```
...
for(b in seq_along(boot_medians)){
    x_boot <- sample(x_small_hat, replace=TRUE)
