...
sample(x, 2, replace=FALSE) #sample
```
...
sample_means
```
For more on loops, see <00_01_FirstSteps.qmd#loops.>.
:::
...
The figure above suggests that the sample mean is centered on the true mean and gets less variable as $n$ grows; the second of these observations is the Law of Large Numbers.

:::{.callout-important icon=false title="Key Definition" collapse="false"}
The *Law of Large Numbers* (LLN) says the sample mean concentrates around the population mean as $n$ grows: with more data, the sampling distribution of $\hat{M}$ tightens around the true $\mu$.
:::
...
The LLN tells us the sampling distribution of $\hat{M}$ tightens around the true mean; the next question is what *shape* it takes as it tightens.
...
The Law of Large Numbers generalizes to many other statistics, like the `median` or `sd` from [Numerical Statistics](01_03_DescriptiveStatistics.qmd).^[When a statistic converges in probability to the quantity they are meant to estimate, they are called *consistent*.] 
...
for(n in c(5, 50, 500)){
    ...
    for(i in seq_along(sample_props_n)){
        x <- sample(1:5, size=n, prob=c(1:5)/15, replace=TRUE)
        p3 <- mean(x==3)
        sample_props_n[i] <- p3
    }
    hist(sample_props_n,
...
In fact, the Glivenko-Cantelli Theorem (GCT) shows entire empirical distribution converges: the ECDF gets increasingly close to the CDF as the sample sizes grow.
...
axis(1, sample_mean, expression(hat(M)), col=rgb(1, 0, 0, .8))

### Bootstrap samples
set.seed(2)  ## Cherry Pick Some Good Ones
B <- lapply(1:20, function(b) { sample(sample_dat, replace=TRUE) })
temp <- lapply(B, hist, breaks=seq(-6, 8, length=21), plot=FALSE)
Bii <- which(sapply(temp, function(x) max(x[['counts']])) <= 5)
## Plot the Cherries
par(fig=c(divideX[1] * (0/3 + 1/3*Fraction2), divideY3[3:4]), new=TRUE)
PlotSample(B[[Bii[1]]], sample_mean, lineMu=FALSE, star=TRUE)
par(fig=c(divideX[1] * (1/3 + 1/3*Fraction2), divideY3[3:4]), new=TRUE)
PlotSample(B[[Bii[3]]], sample_mean, lineMu=FALSE, star=TRUE)
par(fig=c(divideX[1] * (2/3 + 1/3*Fraction2), divideY3[3:4]), new=TRUE)
PlotSample(B[[Bii[2]]], sample_mean, lineMu=FALSE, star=TRUE)
...
Bmeans <- sapply(1:999, function(b) {
    dat_b <- sample(sample_dat, replace=TRUE) # c.f. jackknife
    mean(dat_b)
})
hist(Bmeans, xlim=c(-3, 3), ylim=c(0, 1), breaks=61, border=NA,
     axes=FALSE, col='gray', freq=FALSE, main=NA, yaxs='i',
     xlab='', ylab='')
abline(v=sample_mean, col=rgb(1, 0, 0, .8), lty=2)
abline(v=mean(populationMean))
axis(1, c(-3, 3))
axis(1, at=sample_mean, expression(hat(M)), col=rgb(1, 0, 0, .8))
...
legend(.01, .9, lty=1:2, col=c(rgb(0, 0, 0, .8), rgb(1, 0, 0, .8)), legend=expression(mu, hat(M)))
```
...
```{r}
small_dat <- c(1, 6, 7, 22)
jackknife_meds <- rep(NA, length(small_dat))
for(i in seq_along(jackknife_meds)){
    jackknife_meds[i] <- median(small_dat[-i])
}
jackknife_meds
mean(jackknife_meds)
```
...
set.seed(1)
small_dat <- c(1, 6, 7, 22)
bootstrap_meds <- rep(NA, 8)
for(b in seq_along(bootstrap_meds)){
    dat_b <- sample(small_dat, replace=TRUE)
    bootstrap_meds[b] <- median(dat_b)
}
bootstrap_meds
```
:::
...
With $\hat{F} \approx F$, each bootstrap sample is approximately a random sample.
...
x_probs <- c(1/4, 3/4)
# sample draws
coin_sample <- sample(x, prob=x_probs, 1000, replace=TRUE)
Fhat <- ecdf(coin_sample)

x_probs_boot <- c(Fhat(0), 1-Fhat(0))
x_probs_boot # approximately the theoretical value
coin_resample <- sample(x, prob=x_probs_boot, 999, replace=TRUE)
...
\begin{eqnarray}
\bar{\hat{M}}^{\text{boot}} &=& \frac{3 + 5 + 6 + 7 + 9}{5} = 6 \\
...
$\hat{SE}^{\text{jack}} = \sqrt{ \frac{n-1}{n} \sum_{i}^{n} \left[\hat{M}_{i}^{\text{jack}} - \bar{\hat{M}}^{\text{jack}} \right]^2 } \approx \hat{sd}(\hat{M}_{i}^{\text{jack}}) \sqrt{n}$.
...
for(n in seq_along(Nseq)){
    ...
    for(b in seq(1, B)){
    	b_id <- sample(n_id, size=n, replace=TRUE)
        x_b <- sample_dat[b_id]
...
To generate a random variable using inverse sampling, first sample $p$ from a uniform distribution and then find the associated quantile  quantile function $\hat{F}^{-1}(p)$.^[Drawing random uniform samples with computers is actually quite complex and beyond the scope of this course.]
...
Afterwhich, we sample $p$ from a uniform distribution and then find the associated quantile.
...
In the next chapter we formalize the link between sample statistics and population parameters: estimate, estimator, and parameter.
