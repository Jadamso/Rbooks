...
```
In this example, we consider a range for $Y_{i}$ rather than for $m(X_{i})$. These intervals also take into account the residuals --- the variability of individuals around the mean. 
```{r}
...
\end{eqnarray}
where $\hat{y}_{[i]}(\mathbf{X},\mathbf{H})$ is the model predicted value at $\mathbf{X}_{i}$ based on a dataset that excludes $\mathbf{X}_{i}$, and $\mathbf{H}$ is matrix of bandwidths. With weighted multiple OLS, for example, the matrix is
\begin{eqnarray}
