...
The Law of Large Numbers generalizes to many other statistics, like `median` or `sd`.^[When a statistic converges in probability to the quantity they are meant to estimate, they are called *consistent*.] In fact, the Glivenko-Cantelli Theorem (GCT) shows entire empirical distribution converges: the ECDF gets increasingly close to the CDF as the sample sizes grow. This result is often termed the *The Fundamental Theorem of Statistics*.
