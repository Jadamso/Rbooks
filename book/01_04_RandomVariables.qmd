...
#### **Bernoulli**. {-}
Think of a Coin Flip: Heads or Tails with equal probability. In general, a Bernoulli random variable denotes heads as the event $x=1$ and tails as the event $x=0$, and allows the probability of heads to vary.
\begin{eqnarray}
X_{i} &\in& \{0,1\} \\
Prob(X_{i} =0) &=& 1-p \\
Prob(X_{i} =1) &=& p \\
