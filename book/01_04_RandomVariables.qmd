...
Consider a continuous random variable equally likely to land anywhere in $[0,1]$, so its density is flat: $f(x)=1$ for $x \in [0,1]$.

To build $F$ from $f$, accumulate area. The area under $f$ from $0$ to $x$ is a rectangle of width $x$ and height $1$, so $F(x)=x$.

To recover $f$ from $F$, take the slope. The CDF $F(x)=x$ is a straight line of slope $1$, so $f(x)=1$.
```{r}
x <- seq(0, 1, by=0.01)
par(mfrow=c(1, 2))
# Density f(x): flat
plot(x, dunif(x), type='l', col=rgb(0, 0, 1, .8),
    ylim=c(0, 1.2), xlab='x', ylab='f(x)', main=NA)
title('Density f(x)', font.main=1)
# CDF F(x): accumulated area under f
plot(x, punif(x), type='l', col=rgb(0, 0, 1, .8),
...
```{r}
# Probability a score exceeds 90, when mu=70 and sigma=10
1 - pnorm(90, mean=70, sd=10)
```
Recompute the probability for the other two cases and compare.
:::
