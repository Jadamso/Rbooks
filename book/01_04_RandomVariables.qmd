...
Here is an example of the [Bernoulli distribution](https://en.wikipedia.org/wiki/Bernoulli_distribution)
```{r}
...
Note that the discrete [uniform distribution](https://en.wikipedia.org/wiki/Discrete_uniform_distribution) generalizes to arbitrary intervals, although we will not exploit the generalization in this class.
...
Note that the continuous [uniform distribution](https://en.wikipedia.org/wiki/Continuous_uniform_distribution) generalizes to an arbitrary interval, $X_{i} \in [a,b]$. What is the probability of a value larger than $0.25$ when $a=-b=2$? First use the computer to suggest an answer and then show the answer mathematically.
...
#### **Normal (Gaussian)**. {-}
Any number on the real line, with bell shaped probabilities. The distribution is mathematically complex, but we will encounter it again and again and again. The density function $f$ has two parameters $\mu \in (\infty,\infty)$ and $\sigma > 0$
\begin{eqnarray}
X_{i} &\in& (\infty,\infty) \\
