...
Often, the PDF helps you intuitively understand a random variable whereas the CDF helps you calculate numerical values. This is because probabilities are depicted as areas in the PDF and the CDF accumulates those areas. For example, $Prob(X_{i} \leq 1)$ is depicted by the PDF as the area under $f(x)$ from the lowest possible value until $x=1$, which is numerically calculated simply as $F(1)$.
