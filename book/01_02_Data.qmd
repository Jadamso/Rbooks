...
The hat marks a value computed from observed data; we will contrast it with theoretical quantities in [Population Statistics](https://jadamso.github.io/Rbooks/01_06_PopStatistics.html#statistical-theory).
...
The same bandwidth idea reappears in the kernel density below.
...
:::{.callout-note icon=false collapse="true" title="Must Know"}
For example, if $X=(0,0,0.02,3,5)$ then the median is $0.02$, the lower quartile is $0$, and the upper quartile is $3$. (The number $0$ is also special: the most frequent observation is called the *mode*, which we revisit in [Numerical Statistics](https://jadamso.github.io/Rbooks/01_03_DescriptiveStatistics.html#mode-and-share-concentration).)
...
These univariate visualization tools extend to examining relationships between two variables. See [Bivariate Distributions](https://jadamso.github.io/Rbooks/02_10_BivariateDistributions.html) for how histograms, ECDFs, and boxplots adapt to bivariate data.


## Exercises

1. Classify each of the following as cardinal (discrete or continuous) or factor (ordered or unordered): (a) temperature in Celsius, (b) letter grade on a transcript, (c) number of siblings, (d) province of residence. Briefly explain each choice.

2. The built-in dataset `mtcars` has a column `mpg`. Compute the histogram density by hand for the bins $(10,15]$, $(15,20]$, $(20,25]$, $(25,30]$, $(30,35]$. That is, for each bin count the number of observations, then divide by $n \times 2h$ where $2h=5$. Verify your answer matches `hist(mtcars$mpg, breaks=seq(10,35,by=5), freq=F)`.

3. Load `USArrests` in R. Compute the ECDF of the `Assault` variable and use it to find the proportion of states with 200 or fewer assault arrests. Then find the upper quartile of `Assault` using `quantile`.
