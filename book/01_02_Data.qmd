...
Factor data are *ordered* if the categories have a natural ranking and *unordered* (or *categorical*) otherwise.
:::
...
:::{.callout-important title="Definition" collapse="false"}
A *histogram density* divides the range of the data into bins of equal half-width $h$ and reports, for each bin, the number of observations divided by $n \cdot 2h$.
This rescaling makes the total area of all bars equal to one, so bar heights have units of probability per unit of $x$.
:::

For continuous data, the *[histogram](https://en.wikipedia.org/wiki/Histogram)* is the workhorse picture of where observations are concentrated. Rescaling by $n \cdot 2h$ matters because it makes histogram heights comparable across samples of different sizes and lets us interpret the bar height as a *density*: the proportion of data per unit of $x$.
...
The smoothness comes from using a *kernel function* to weight the contribution of each data point. The simplest choice is the "uniform" (or "rectangular") kernel, which places a weight of $1/2$ on every point inside the interval $\left[ x-h, x + h\right]$ and zero outside:
\begin{eqnarray}
\label{eqn:uniform}
...
The ECDF is a *step function*: flat between observations, jumping vertically at each one (see this [ECDF walkthrough](https://library.virginia.edu/data/articles/understanding-empirical-cumulative-distribution-functions) for visual examples). It contains exactly the same information as the sorted dataset, just packaged as a function of $x$ rather than a list of values.
...
The *median* is the $0.5$ quantile, the *quartiles* are at $p=0.25$ and $p=0.75$, and the *deciles* are at $p=0.1, 0.2, \ldots, 0.9$.
:::

We use quantiles to summarize different parts of the distribution at once:
