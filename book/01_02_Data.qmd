...
:::{.callout-note icon=false collapse="true" title="Must Know"}
For example, if $X=(0,0,0.02,3,5)$ then the median is $0.02$, the lower quartile is $0$, and the upper quartile is $3$. (The number $0$ is also special: the most frequent observation is called the *mode*, which we revisit in [Numerical Statistics](01_03_DescriptiveStatistics.qmd#mode-and-share-concentration.).)
...
These univariate visualization tools extend to examining relationships between two variables. See [Bivariate Distributions](02_10_BivariateDistributions.qmd) for how histograms, ECDFs, and boxplots adapt to bivariate data.
