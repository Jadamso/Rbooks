...
\end{aligned}$$
where $\mathbf{1}( x=y )$ is an indicator function that equals $1$ if $x=y$ and $0$ otherwise.
The marginal distribution of the second coin is
$$\begin{aligned}
Prob(Y_{i} = 0)
...
For example, suppose $(X_{i},Y_{i})$ is bivariate normal with means $(\mathbb{E}[X_{i}], \mathbb{E}[Y_{i}])$, variances $(\mathbb{V}[X_{i}], \mathbb{V}[Y_{i}])$ and covariance $\mathbb{C}[X_{i}, Y_{i}]$.
...
hist(MVdat1[, 2], col=rgb(0, 0, 1, 0.5),
    add=TRUE, breaks=xbks, border=NA)
```
...
When we know how the data are generated theoretically, we can often compute the theoretical value of the most basic and often-used bivariate statistic: the Pearson correlation.
To see this, we focus on two discrete random variables, first showing their covariance and then their correlation.
...
n1  <- length(y1_hat)
n2  <- length(y2_hat)
se_hat <- sqrt(var(y1_hat)/n1 + var(y2_hat)/n2);
t_obs <- d/se_hat

# Alt estimate
v1_hat  <- var(y1_hat)
v2_hat  <- var(y2_hat)
v_hat   <- ((n1-1)*v1_hat + (n2-1)*v2_hat)/(n1+n2-2)
d_se <- sqrt(v_hat*(1/n1+1/n2))
# t stat
t_stat <- d/d_se
t_stat
```
