...
A *function* is a named recipe that takes input values (called arguments), performs a calculation, and returns an output.
...
```{r}
1*(x == 4)                # <1>
sum(x == 4, na.rm=TRUE)   # <2>
mean(x == 4, na.rm=TRUE)  # <3>
```
